...
def calculate_mean(dates, values):
    new_values = []
    new_args = []
    i = 0
    while i < len(dates):
...
async def get_pair(url):
    async with ClientSession() as session:
        d = await get_data(session, url)
        ...
        new = calculate_mean(dates, values)
    return new
...
async def main():
    gbp1 = await get_pair(gbp21_url)
    gbp2 = await get_pair(gbp22_url)
    usd1 = await get_pair(usd21_url)
    usd2 = await get_pair(usd22_url)
