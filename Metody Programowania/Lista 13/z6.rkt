...
(define-type Binding
  (bind [name : Symbol]
        [val : Promise])) ; zamiast wartości, będziemy przechowywać opóźnione obliczenie
...
(define-type-alias Promise (Boxof Thunk)) ; new
...
(define (extend-env [env : Env] [x : Symbol] [e : Exp] [env1 : Env]) : Env
  (cons (bind x (delay e env1)) env)) ; opakowujemy wartość w thunk i zapisujemy jako Promise w środowisku
...
(define (lookup-env [n : Symbol] [env : Env]) : Value
  (type-case (Listof Binding) env
    ...
    [(cons b rst-env) (cond
                        [(eq? n (bind-name b))
                         (force (bind-val b))] ; wyliczamy wartość w momencie gdy ją potrzebujemy
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(varE x)
     (lookup-env x env)]
    [(letE x e1 e2)
      (eval e2 (extend-env env x e1 env))] ; rozszerzamy środowisko o odroczone obliczenia i ewaluujemy resztę obliczeń
    [(lamE x b)
     (funV x b env)]
    [(appE e1 e2)
     (apply (eval e1 env) e2 env)])); nie liczymy e2 w tym miejscu

(define (apply [v1 : Value] [e2 : Exp] [env1 : Env]) : Value
  (type-case Value v1
    [(funV x b env)
     (eval b (extend-env env x e2 env1))] ; obliczamy b w rozszerzonym środowisku
