...
(define (extend-env [env : Env] [x : Symbol] [e : Exp]) : Env
  (cons (bind x (delay e env)) env)) ; opakowujemy wartość w thunk i zapisujemy jako Promise w środowisku
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(varE x)
     (lookup-env x env)]
    [(letE x e1 e2)
      (eval e2 (extend-env env x e1))] ; rozszerzamy środowisko o odroczone obliczenia i ewaluujemy resztę obliczeń
    [(lamE x b)
     (funV x b env)]
    [(appE e1 e2)
     (apply (eval e1 env) e2)])); nie liczymy e2 w tym miejscu

(define (apply [v1 : Value] [e2 : Exp]) : Value
  (type-case Value v1
    [(funV x b env)
     (eval b (extend-env env x e2))] ; obliczamy b w rozszerzonym środowisku
