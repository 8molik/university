...
(define-type Value
  (numV [n : Number])
  (boolV [b : Boolean])
  (funV [x : Symbol] [e : Exp]) ; usunięto środowisko env
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(lamE x b)
     (funV x b)] ; nowe, teraz zmienne nie są związane w trakcie definiowania funkcji
    [(appE e1 e2)
     (apply (eval e1 env) (eval e2 env) env)])) ; przekazujemy środowisko do apply,
                                                ; dzięki czemu możemy korzystać z aktualnego środowiska w trakcie ewaluacji

(define (apply [v1 : Value] [v2 : Value] [env : Env]) : Value ; teraz apply przyjmuje środowisko
  (type-case Value v1
    [(funV x b) ; nowe
...
(module+ test
  ...
  (test (run `{let fact {lambda {n} ; nie potrzebujemy letrec, let działa jak letrec
                     {if {= n 0}
                          1
                          {* n {fact {- n 1}}}}}
          {fact 5}})
...
; wcześniej wartości zmiennych były związane na etapie definiowania funkcji,