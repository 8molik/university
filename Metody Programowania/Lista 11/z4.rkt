...
(define (new-name) ;generowanie nowych nazw zmiennych
  (begin
...
(define (translate-back [e : ExpA] [env : (Listof Symbol)]) : Exp
  (type-case ExpA e
    [(numA n)
     (numE n)] 
    [(opA o l r)
     (opE o (translate-back l env) (translate-back r env))] ;rekurencyjnie tłumaczymy lewe i prawe poddrzewo
    [(ifA b l r)
     (ifE (translate-back b env)
          (translate-back l env)
          (translate-back r env))]
    [(varA address)                                         ;jeżeli mamy wartość z adresem
     (varE (list-ref env address))]                         ;szukamy w środowisku nazwy zmiennej pod adresem, za pomocą list-ref
    [(letA e1 e2)
     (let ([x (new-name)])                                  ;nadajemy nową nazwę zmiennej
       (letE x (translate-back e1 env)                      ;rekurencyjnie wyliczamy jej wartość
           (translate-back e2 (extend-envA env x))))]))     ;tłumaczymy resztę wyrażeń let'a, 

;testy

(module+ test
   (test (eval (parse `{let x 3 {+ x {let x {* x 3} {+ x 2}}}}) mt-env)
         (eval (translate-back (translate (parse `{let x 3 {+ x {let x {* x 3} {+ x 2}}}}) mt-env) mt-env) mt-env))

   (test (eval (parse `{let x 2 {* x {let y 2 {+ x y}}}}) mt-env)
         (eval (translate-back (translate (parse `{let x 2 {* x {let y 2 {+ x y}}}}) mt-env) mt-env) mt-env)))
