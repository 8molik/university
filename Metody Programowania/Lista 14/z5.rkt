...
(define (bindM [c : (M 'a)] [f : ('a -> (M 'b))]) : (M 'b)
  (let ([val*out (f (fst c))]) ;wartość a w monadzie c
    (pair (fst val*out) (append (snd c) (snd val*out)))))
...
(define (showM [c : (M Value)]) : String
    (string-append "value: "
    (string-append (value->string (fst c))
    (string-append ", output: "
                   (list-to-string (snd c))))))

(define (list-to-string xs)
  (foldr (λ (x acc) (string-append (to-string x)
                    (string-append " "
                                   acc))) "" xs))
